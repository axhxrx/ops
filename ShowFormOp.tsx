...
export class ShowFormOp<T extends Record<string, FormItem<unknown>>> extends Op
{
  ...
  constructor(
    private form: Form<T>,
    private options: ShowFormOpOptions = {},
  )
  {
    super();
  }
  ...
  async run(io?: IOContext): Promise<Success<ShowFormOpSuccess<Record<string, unknown>>> | Failure<ShowFormOpFailure>>
  {
    const ioContext = this.getIO(io);

    let result: ShowFormOpSuccess<Record<string, unknown>> | 'canceled' | null = null;
...
if (import.meta.main)
{
  ...
  const contactForm = Form.create({
    ...
    .title('📧 Contact Form')
    .header(InfoPanel.text('Please fill out your contact information'))
    .footer(InfoPanel.text('Fields marked with * are required | Tab: Next | Enter: Submit | Esc: Cancel'));
  ...
  else if (contactResult.ok === false && contactResult.failure === 'canceled')
  {
  ...
  const registrationForm = Form.create({
    ...
    .title('👤 User Registration')
    .header(InfoPanel.lines(
      'Create your account',
      '',
      'Username must be 3-20 characters (letters, numbers, underscore only)',
    ))
    .footer(InfoPanel.text('Tab/Shift+Tab: Navigate | y/n/space: Toggle boolean | Enter: Submit'));
  ...
  else if (registrationResult.ok === false && registrationResult.failure === 'canceled')
  {
  ...
  const settingsForm = Form.create({
    ...
    timeout: FormItem.number('timeout', 30)
      ...
      .validator((v) => v < 1 ? 'Must be at least 1 second' : v > 300 ? 'Maximum 300 seconds' : undefined)
      .labelColor('yellow'),
    ...
    .footer(InfoPanel.columns('Status: Ready', 'Press Enter to save', 'Esc to cancel'));
  ...
  else if (settingsResult.ok === false && settingsResult.failure === 'canceled')
  {
    console.log('\n🚫 Settings not saved');
  }
  ...
  const validationForm = Form.create({
    ...
    .footer(InfoPanel.text('Try submitting with invalid data to see validation errors'));
  ...
  else if (validationResult.ok === false && validationResult.failure === 'canceled')
  {
    console.log('\n🚫 Form canceled');
  }
